...
async function main() {
    ...
    const seller = await db_1.default.user.upsert({
    ...
    const buyer = await db_1.default.user.upsert({
    ...
    const sellerAddress = await db_1.default.orderAddress.create({
        data: {
            user_id: seller.id,
    ...
    await db_1.default.orderAddress.create({
        data: {
            user_id: buyer.id,
    ...
    for (const b of brands) {
        await db_1.default.brand.upsert({
            where: { id: b.id },
    ...
    for (const item of productsData) {
        ...
        for (const v of item.variants) {
            await db_1.default.productVariant.create({
                data: {
        ...
        await db_1.default.productVideo.create({
            data: {
                product_id: product.id,
    ...
    for (const n of newsList) {
        await db_1.default.news.create({ data: n });
    }
