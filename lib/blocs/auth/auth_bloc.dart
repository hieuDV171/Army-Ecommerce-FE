  ...
  AuthBloc({required this.authRepository}) : super(AuthInitial()) {
    ...
    on<LoginButtonPressed>((event, emit) async {
      ...
      try {
        ...
        final responseCode = ResponseCode.fromCode(
          response.code,
        ...
        if (responseCode == ResponseCode.ok && response.data != null) {
          ...
          if (response.data!.avatar != null &&
              response.data!.avatar!.isNotEmpty) {
            await SessionManager.setAvatar(response.data!.avatar!);
          }
          if (response.data!.coverImage != null &&
              response.data!.coverImage!.isNotEmpty) {
            await SessionManager.setCoverImage(response.data!.coverImage!);
          }
          if (response.data!.coverImageWeb != null &&
              response.data!.coverImageWeb!.isNotEmpty) {
            await SessionManager.setCoverImageWeb(
              response.data!.coverImageWeb!,
            );
          }
          logger.i(
            'Login: saved session for username="${response.data!.username}" phone="${event.phoneNumber}"',
          );
        ...
        emit(
          AuthFailure(error: e.toString(), code: ResponseCode.exception.code),
        );
      }
    });
    ...
    on<LogoutButtonPressed>((event, emit) async {
      logger.i(
        'DEBUG LOGOUT BLOC: Received LogoutButtonPressed event, token="${event.token}"',
      );
      emit(AuthLoading());
      ...
      if (event.token.isNotEmpty) {
        ...
        logger.w(
          'DEBUG LOGOUT BLOC: Token is empty, skipping backend logout request.',
        );
      }

      // 2. Xóa session local lập tức và chuyển trạng thái thành công
      logger.i(
        'DEBUG LOGOUT BLOC: Clearing local session via SessionManager...',
      );
      await SessionManager.clearSession();
      logger.i(
        'DEBUG LOGOUT BLOC: Local session cleared successfully. Emitting AuthLogoutSuccess.',
      );
    ...
    on<VerifyOtpPressed>((event, emit) async {
      ...
      try {
        final response = await authRepository.checkSignupCode(
          event.phoneNumber,
          event.code,
        );
        ...
        if (responseCode == ResponseCode.ok) {
          ...
          if (loginCode == ResponseCode.ok && loginResponse.data != null) {
            ...
            if (realUser.coverImage != null &&
                realUser.coverImage!.isNotEmpty) {
              await SessionManager.setCoverImage(realUser.coverImage!);
            }
            if (realUser.coverImageWeb != null &&
                realUser.coverImageWeb!.isNotEmpty) {
              await SessionManager.setCoverImageWeb(realUser.coverImageWeb!);
            }
            logger.i(
              'VerifyOtp (Autologin): saved session for username="${realUser.username}" phone="${event.phoneNumber}"',
            );

            emit(AuthSuccess(user: realUser));
          } else {
            final errMsg = loginResponse.message.isNotEmpty
                ? loginResponse.message
                : loginCode.message;
            emit(
              AuthFailure(
                error: 'Đăng nhập tự động thất bại: $errMsg',
    ...
    on<AppStarted>((event, emit) async {
      ...
      logger.d(
        'AutoLogin: token from SessionManager: ${token != null ? "[REDACTED]" : "null"}',
      );
      ...
      try {
        ...
        logger.d(
          'AutoLogin: local username from SharedPreferences: "$localUsername" phone="$localPhone"',
        );

        final response = await authRepository.getUserInfo(token: token);
        logger.d(
          'AutoLogin: getUserInfo response code=${response.code}, data=${response.data}',
        );
        ...
        if (responseCode == ResponseCode.ok && response.data != null) {
          ...
          final usernameToUse = apiUsername.isNotEmpty
              ? apiUsername
              : localUsername;
          if (apiUsername.isEmpty && localUsername.isNotEmpty) {
            logger.w(
              'AutoLogin: API returned empty username -> using local username "$localUsername"',
            );
          } else {
          ...
          logger.w(
            'AutoLogin: token invalid or response not OK -> clearing session. response.code=${response.code}',
          );
    ...
    on<ForgotPasswordRequested>((event, emit) async {
      ...
      try {
        final response = await authRepository.createCodeResetPassword(
          event.phoneNumber,
        );
        final responseCode = ResponseCode.fromCode(
        ...
        if (responseCode == ResponseCode.ok) {
          ...
          emit(
            AuthFailure(
              error:
                  response['message']?.toString() ?? 'Gửi mã xác thực thất bại',
    ...
    on<ResetPasswordRequested>((event, emit) async {
      ...
      try {
        ...
        if (responseCode == ResponseCode.ok && response.data != null) {
          ...
          logger.i(
            'ResetPassword: saved session for username="${response.data!.username}" phone="${event.phoneNumber}"',
          );
    ...
    on<ChangeInfoRequested>((event, emit) async {
      ...
      try {
        ...
        if (responseCode == ResponseCode.ok && response.data != null) {
          ...
          final userWithToken = response.data!.copyWith(
            token: token,
            active: 1,
          );
    ...
    on<GetUserInfoRequested>((event, emit) async {
      ...
      try {
        ...
        logger.d(
          'GetUserInfo: response code=${response.code}, data=${response.data}',
        );
        ...
        if (responseCode == ResponseCode.ok && response.data != null) {
          ...
          if (userId == null) {
            ...
            if (fetchedUser.coverImage != null &&
                fetchedUser.coverImage!.isNotEmpty) {
              await SessionManager.setCoverImage(fetchedUser.coverImage!);
            }
            if (fetchedUser.coverImageWeb != null &&
                fetchedUser.coverImageWeb!.isNotEmpty) {
    ...
    on<SetUserInfoRequested>((event, emit) async {
      ...
      try {
        ...
        if (responseCode == ResponseCode.ok && response.data != null) {
          ...
          if (updatedUser.coverImage != null &&
              updatedUser.coverImage!.isNotEmpty) {
            await SessionManager.setCoverImage(updatedUser.coverImage!);
          }
          if (updatedUser.coverImageWeb != null &&
              updatedUser.coverImageWeb!.isNotEmpty) {
