...
class OrderBloc extends Bloc<OrderEvent, OrderState> {
  ...
  Future<void> _onDetailRequested(
    ...
    try {
      ...
      final timelineFuture = marketplaceRepository.getOrderTimeline(
        event.orderId,
      );
      ...
      emit(
        state.copyWith(isDetailLoading: false, errorMessage: error.toString()),
      );
    }
  }
  ...
  Future<void> _onActionRequested(
    ...
    try {
      ...
      if (event.actionType == OrderActionType.accept ||
        ...
        if (buyerId == null) {
          final detail = await marketplaceRepository.getOrderDetail(
            event.order.id,
          );
      ...
      switch (event.actionType) {
        case OrderActionType.accept:
          await marketplaceRepository.setAcceptBuyer(
            event.order.id,
            buyerId!,
            true,
          );
        ...
        case OrderActionType.ship:
          await marketplaceRepository.sellerMarkAsShipped(
            event.order.id,
            buyerId: buyerId!,
          );
        ...
        case OrderActionType.cancel:
          await marketplaceRepository.cancelOrder(
            event.order.id,
            reason: event.reason,
          );
          msg = 'Đã hủy đơn hàng';
        ...
        case OrderActionType.refund:
          await marketplaceRepository.refundOrder(
            event.order.id,
            reason: event.reason,
          );
      ...
      emit(state.copyWith(isActionInProgress: false, successMessage: msg));
    } catch (error) {
      emit(
        state.copyWith(
  ...
  Future<void> _onEditRequested(
    ...
    emit(state.copyWith(isActionInProgress: true, clearMessages: true));
    try {
      final updatedData = await marketplaceRepository.editOrder(
        event.orderId,
        event.data,
      );
      ...
      if (currentDetail != null) {
        ...
        final updatedAddress =
            updatedData['address']?.toString() ?? currentDetail.buyerAddress;
